...
def test_single_tool_call(chat_template):
    ...
    tool_call_section = result.split('<|start_header_id|>assistant<|end_header_id|>')[1].split('<|eot_id|>')[0]
    
    # Should contain proper JSON structure
    assert '{"name": "get_weather", "parameters": {"location": "Paris", "unit": "celsius"}}' in tool_call_section
...
def test_tool_response(chat_template):
    ...
    tool_response_section = result.split('<|start_header_id|>ipython<|end_header_id|>')[1].split('<|eot_id|>')[0]
        ...
        messages=[
            {
                ...
                'content': [
                    ...
                    {'type': 'image', 'image_url': {'url': 'https://example.com/weather.png'}}
                ]
            }
        ]
    )
...
def test_full_tool_conversation(chat_template):
    ...
    # Verify tool call
    tool_call_section = result.split('<|start_header_id|>assistant<|end_header_id|>')[1].split('<|eot_id|>')[0]
    assert '{"name": "get_weather", "parameters": {"location": "Paris"}}' in tool_call_section
    ...
    final_assistant_section = result.split('<|start_header_id|>assistant<|end_header_id|>')[2].split('<|eot_id|>')[0]
...
def test_tool_in_user_message(chat_template):
    ...
    tools_section = result.split('<|start_header_id|>user<|end_header_id|>')[1].split('<|eot_id|>')[0]
